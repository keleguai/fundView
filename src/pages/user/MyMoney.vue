<template>
  <div style="background: url(/static/bk/bk.png) repeat-x #fff;">
    ...
    <md-dialog :md-active.sync="doubleCheck" :style="$store.state.isAndroid==true?'width:100%':'width:40%'">
      <md-dialog-content>
        <p style="color:#555">
          确定要取消该基金的定投吗
        </p>
        <md-card-actions>
          <md-button class="md-primary" @click="doubleDelete(id)">
            确定
          </md-button>
          <md-button @click="doubleCheck=false"class="md-primary">取消</md-button>
        </md-card-actions>
      </md-dialog-content>
    </md-dialog>
      ...
      <div
        ...
        <md-list>
          <md-list-item style="border-bottom: 1px solid #f4f4f4;">
            <md-icon class="md-primary" v-show="user_flag&&!fixedFlag">done</md-icon>
          ...
          <md-list-item style="border-bottom: 1px solid #f4f4f4;">
            <md-icon class="md-primary" v-show="!fixedFlag&&select_buy_flag&&fund_flag">done</md-icon>
            <a class="md-list-item-text" style="color: #999;text-align: center;text-decoration:none"
          ...
          <md-list-item style="border-bottom: 1px solid #f4f4f4;">
            <md-icon class="md-primary" v-show="(!(select_buy_flag&&fund_flag))&&fixedFlag">done</md-icon>
            <a class="md-list-item-text" style="color: #999;text-align: center;text-decoration:none"
               @click="getFixedFund(1)" href="javascript:void(0)">我的定投</a>
          </md-list-item>
        </md-list>
                ...
                <template v-else v-for="(fund,index) in funds">
                  <div class="md-layout md-gutter md-alignment-top-center"
                    ...
                    <div class="md-layout-item md-large-size-30 md-medium-size-30 md-small-size-30 md-xsmall-size-30">
                      ...
                      <p class="li" v-show="fund.sendTime">开始时间：<b>{{ $myapi.parseMyDate(fund.sendTime)}}</b></p>
                    ...
                    <div class="md-layout-item md-large-size-30 md-medium-size-30 md-small-size-30 md-xsmall-size-30">
                      ...
                      <p class="li" v-show="!user_flag&&!fund.sellTime&&!fixedFlag">状态：<b style="color: #555">{{fund.cancel?"已撤销":""
                        + !fund.cancel&&fund.confirmSign?"已成交":"买入待确认价值"}}</b>
                      </p>
                      <p class="li" v-show="!fixedFlag&&!user_flag&&fund.sellTime">状态：<b style="color: #555">{{fund.cancel?"已撤销":""
                        + !fund.cancel&&fund.confirmSign?"已成交":"卖出待确认净值"}}</b>
                      </p>
                      <a class="li" v-show="!fixedFlag&&!user_flag&&fund.sellTime&&(fund.confirmSign==0&&!fund.cancel)"
                      ...
                      <p class="li" v-show="fund.money">金额：<b>{{ fund.money}}元</b></p>
                      <p class="li" v-show="fund.loopTime">周期：<b>{{ (fund.loopTime==7?'每周':''+fund.loopTime==14?'每两周':''+fund.loopTime==28?'每月':'')+"的周"+(fund.weekDay+1)}}</b></p>
                      <a class="li" v-on:click="sell(index)" v-show="user_flag&&!fixedFlag"
                         style="float: left;text-align: right">卖出</a>

                      <a style="float: left;text-align: right" class="li" v-on:click="deleteFixed(fund.id)"  v-show="fixedFlag">
                          取消定投
                      </a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      doubleDelete(id){
        let _this = this
        _this.doubleCheck = false
        this.$myapi.get("/fixed_fund/delete/"+id, {}, function (res) {
          _this.$alert.show(100,"已取消该定投")
          _this.getFixedFund(_this.page)
        })
      },
      getFixedFund(page){
        let _this = this
        _this.page = page
        _this.fixedFlag = true
        _this.fund_flag = false
        this.$myapi.get("/fixed_fund/find/me/"+_this.page, {}, function (res) {
